...
// Chat Renderer
const ChatView = ({ name, color, project }) => (
...
// iframe Renderer
const IframeView = ({ sandboxUrl, name }) => (
...
export const AgentSandbox = ({ project }) => {
    ...
    return (
        <div className="h-full">
            {sandboxType === 'terminal' && <TerminalView name={name} color={color} />}
            {sandboxType === 'chat' && <ChatView name={name} color={color} project={project} />}
            {sandboxType === 'iframe' && <IframeView sandboxUrl={sandboxUrl} name={name} />}
        </div>
    );
};
